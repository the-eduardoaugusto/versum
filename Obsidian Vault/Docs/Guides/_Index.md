---
title: "Guides"
section: "Docs"
subsection: "Guides"
tags: [versum, guides, moc]
up: "[[Docs/_Index]]"
related: []
depth: 1
---

# 📚 Guides

Guias atemporais de desenvolvimento — convenções, padrões técnicos, fluxos.

## Conteúdo

| Guia | Descrição |
|:--|:--|
| [[Docs/Naming Convention\|Naming Convention]] | DB `snake_case`, API `camelCase`, view models |
| [[Docs/API Development\|API Development]] | Estrutura de módulos, view models, erros, testes |
| [[Docs/API Response Standardization\|API Response Standardization]] | Envelope unificado de resposta |
| [[Docs/Git Flow\|Git Flow]] | Branches, conventional commits, sandbox IA |
| [[Docs/LGPD Implementation\|LGPD Implementation]] | Middleware de consentimento, deleção, portabilidade |
| [[Docs/Incident Response Plan\|Incident Response Plan]] | Detecção, triagem, contenção, notificação ANPD |

---

◀ [[Docs/_Index|🏠 Home]] · [[Docs/Apps/_Index|Apps]] ▶
