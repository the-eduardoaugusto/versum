...
└── hooks/
    ├── use-journey-feed.ts       # TanStack Query para buscar capítulos
    ├── use-active-chapter.ts     # IntersectionObserver para marcar como lido
    ├── use-keyboard-navigation.ts # Navegação com arrow keys
    ├── use-chapter-pagination.ts  # Página ativa do capítulo
    └── use-journey-progress.ts    # Progresso da jornada
```
...
Marca o capítulo atual como lido e avança o ponteiro.

**Sem body**, apenas marca e retorna novo progress.
...
- **Debounce:** 500ms antes de marcar como lido
