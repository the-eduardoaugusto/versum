...
export const TRUNCATABLE_TABLES = [
  ...
  { name: "journey_readings", label: "Journey — leituras" },
...
const NAMES = new Set<string>(TRUNCATABLE_TABLES.map((t) => t.name));

export function assertTruncatableTableNames(
  names: string[],
): asserts names is TruncatableTableName[] {
...
export const TRUNCATE_PRESETS: Record<
  string,
  {
    title: string;
    description: string;
    tables: readonly TruncatableTableName[];
  }
> = {
  all: {
  ...
  interactions: {
    ...
    tables: ["journey_readings", "likes", "marks"],
  },
  auth: {
  ...
  users_chain: {
    ...
    tables: ["journey_readings", "likes", "marks", "sessions", "users"],
  },
};
