...
export class UsersCommonSchemasV1 {
  ...
  static readonly consentLogSchema = z
    .object({
      id: z
        .string()
        .openapi({ description: "ID do registro de consentimento" }),
      userId: z.string().openapi({ description: "ID do usuário" }),
      purpose: z
        .string()
        .openapi({ description: "Finalidade do consentimento" }),
      granted: z
        .boolean()
        .openapi({ description: "Se o consentimento foi concedido" }),
      createdAt: z
        .string()
        .datetime()
        .openapi({ description: "Data do registro" }),
    })
  ...
  static readonly exportUserDataResponseSchema = createSuccessResponseSchema(
